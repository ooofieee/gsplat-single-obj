...
class Config:
    ...
    data_dir: str = "/home/ooofieee/co3d_data/couch_colmap_rgbmask"
    # Downsample factor for the dataset
    data_factor: int = 4
    # Directory to save results
    result_dir: str = "results/couch_customized"
    ...
    # Path to CO3D dense point cloud PLY (overrides SfM points for initialization)
    co3d_pc_path: Optional[str] = None
    # Max points to keep from CO3D point cloud (random subsample). 0 = keep all.
    co3d_pc_max_points: int = 30_000
    # Mask contribution audit: suppress out-of-mask Gaussians after densification.
    # Set mask_audit_every to strategy.refine_every (e.g. 100) to audit after every densification step.
    mask_audit_every: int = 0       # 0 = disabled
    mask_audit_views: int = 8       # training views to sample per audit
    mask_audit_ratio: float = 0.1   # min fraction of visible views inside mask to keep GS
...
class MaskedObjectDataset(Dataset):
    """COLMAP dataset that uses SAM-masked RGB targets and per-image masks."""
    ...
    def __init__(
        ...
        masked_image_dir: str = "masked_images",
        object_mask_dir: str = "masks",
    ):
        super().__init__(
            parser,
            split=split,
        ...
        self.masked_image_root = Path(parser.data_dir) / masked_image_dir
        self.object_mask_root = Path(parser.data_dir) / object_mask_dir
        if not self.masked_image_root.exists():
            raise ValueError(f"Masked image folder {self.masked_image_root} does not exist.")
        if not self.object_mask_root.exists():
            raise ValueError(f"Object mask folder {self.object_mask_root} does not exist.")

    @staticmethod
    def _stem_png(root: Path, image_name: str) -> Path:
        path = root / f"{Path(image_name).stem}.png"
        if not path.exists():
            raise FileNotFoundError(f"Expected masked CO3D file {path}")
        return path
    ...
    def _resize_rgb(image: np.ndarray, width: int, height: int) -> np.ndarray:
        if image.shape[:2] == (height, width):
            return image
        return np.array(
            Image.fromarray(image).resize((width, height), Image.BICUBIC)
        )
    ...
    def __getitem__(self, item: int) -> Dict[str, Tensor]:
        ...
        image = imageio.imread(self._stem_png(self.masked_image_root, image_name))[..., :3]
        object_mask = imageio.imread(self._stem_png(self.object_mask_root, image_name))
        if object_mask.ndim == 3:
            object_mask = object_mask[..., 0]

        image = self._resize_rgb(image, width, height)
        object_mask = self._resize_mask(object_mask, width, height) > 0
        ...
        if len(params) > 0:
            ...
            object_mask = cv2.remap(
                object_mask.astype(np.uint8), mapx, mapy, cv2.INTER_NEAREST
            ).astype(bool)
        ...
        if camera_mask is not None:
            object_mask = np.logical_and(object_mask, camera_mask)
        ...
        data = {
            ...
            "mask": torch.from_numpy(object_mask).bool(),
        }
...
class Runner:
    ...
    def __init__(
        ...
        if cfg.data_type == "ncore":
            ...
            self.trainset = MaskedObjectDataset(
                self.parser,
                split="train",
                patch_size=cfg.patch_size,
                load_depths=cfg.depth_loss,
            )
            self.valset = MaskedObjectDataset(self.parser, split="val")
        self.scene_scale = self.parser.scene_scale * 1.1 * cfg.global_scale
        print("Scene scale:", self.scene_scale)

        # If a CO3D dense point cloud is provided, use it instead of SfM sparse points.
        if cfg.co3d_pc_path is not None:
            from plyfile import PlyData
            from scipy.linalg import orthogonal_procrustes

            plydata = PlyData.read(cfg.co3d_pc_path)
            vert = plydata["vertex"]
            co3d_points = np.stack(
                [vert["x"], vert["y"], vert["z"]], axis=-1
            ).astype(np.float32)
            co3d_colors = np.stack(
                [vert["red"], vert["green"], vert["blue"]], axis=-1
            ).astype(np.uint8)

            # Compute CO3D → COLMAP alignment using camera centers.
            # CO3D point cloud lives in CO3D's world; COLMAP cameras live in COLMAP's
            # world. We find a similarity transform between the two coordinate systems
            # by matching camera centers from both.
            co3d_centers = {}
            colmap_centers = {}
            import re

            # CO3D camera centers (from frame annotations)
            ...
            pc_path = Path(cfg.co3d_pc_path)
            co3d_category_dir = pc_path.parent.parent  # .../toytruck/
            co3d_root = co3d_category_dir.parent  # .../co3d_data/
            co3d_category = co3d_category_dir.name  # e.g. "toytruck"

            frames = load_dataclass_jgzip(
                str(co3d_root / co3d_category / "frame_annotations.jgz"),
                TypingList[FrameAnnotation],
            )
            seq_name = pc_path.parent.name  # e.g. "190_20494_39385"
            for f in frames:
                if f.sequence_name == seq_name:
                    R = np.asarray(f.viewpoint.R, dtype=np.float64)
                    T = np.asarray(f.viewpoint.T, dtype=np.float64)
                    co3d_centers[f.frame_number] = -R @ T

            # COLMAP camera centers (from sparse model, unnormalized)
            from pycolmap import SceneManager

            sparse_dir = Path(cfg.data_dir) / "sparse" / "0"
            mgr = SceneManager(str(sparse_dir))
            mgr.load_images()
            for img in mgr.images.values():
                fn = re.search(r"frame(\d+)\.", img.name)
                if fn:
                    frame_num = int(fn.group(1))
                    colmap_centers[frame_num] = -img.R().T @ img.t

            # Compute similarity transform: CO3D → COLMAP
            common_frames = sorted(
                set(co3d_centers.keys()) & set(colmap_centers.keys())
            )
            if len(common_frames) < 3:
                print(
                    "[CO3D PC] Warning: too few matching cameras "
                    f"({len(common_frames)}) for alignment — using SfM init."
                )
            else:
                co3d_arr = np.array([co3d_centers[k] for k in common_frames])
                colmap_arr = np.array([colmap_centers[k] for k in common_frames])

                c_mean = co3d_arr.mean(axis=0)
                l_mean = colmap_arr.mean(axis=0)
                c_c = co3d_arr - c_mean
                l_c = colmap_arr - l_mean
                scale = np.sqrt(np.sum(l_c**2) / np.sum(c_c**2))
                R_align, _ = orthogonal_procrustes(c_c * scale, l_c)
                t_align = l_mean - scale * R_align @ c_mean
                residuals = np.linalg.norm(
                    (co3d_arr * scale) @ R_align.T + t_align - colmap_arr, axis=-1
                )
                print(
                    f"[CO3D PC] Alignment: scale={scale:.4f}, "
                    f"residual mean={residuals.mean():.3f} max={residuals.max():.3f}"
                )

                # Transform CO3D points: CO3D world → COLMAP world
                co3d_points = (co3d_points * scale) @ R_align.T + t_align

            # Apply the same world-space normalization used for cameras.
            T = self.parser.transform  # (4, 4), COLMAP world → normalized world
            if not np.allclose(T, np.eye(4)):
                co3d_points_h = np.concatenate(
                    [co3d_points, np.ones((len(co3d_points), 1))], axis=-1
                )
                co3d_points = (co3d_points_h @ T.T)[:, :3]

            self.parser.points = co3d_points
            self.parser.points_rgb = co3d_colors

            # Random subsample if too dense
            if cfg.co3d_pc_max_points > 0 and len(self.parser.points) > cfg.co3d_pc_max_points:
                idx = np.random.choice(
                    len(self.parser.points), cfg.co3d_pc_max_points, replace=False
                )
                self.parser.points = co3d_points[idx]
                self.parser.points_rgb = co3d_colors[idx]
                print(f"[CO3D PC] Subsampled to {cfg.co3d_pc_max_points} points")

            print(
                f"Replaced SfM points with CO3D dense point cloud: "
                f"{len(self.parser.points)} points"
            )

        # Filter SfM points by mask projection consistency.
        # Points that never project inside the object mask receive zero gradient
        # during training and will never be pruned — they cause distractor artifacts.
        # We project each SfM point to all training views and keep only those that
        # land inside the mask for at least mask_init_ratio of their visible views.
        if self.cfg.data_type != "ncore":
            from PIL import Image

            mask_dir = Path(cfg.data_dir) / "masks"
            if mask_dir.exists():
                print("[MaskFilter] Filtering SfM points by mask projection...")
                points = self.parser.points  # (N, 3) in normalized space
                N = len(points)
                hit_count = np.zeros(N, dtype=np.float64)
                vis_count = np.zeros(N, dtype=np.float64)

                for idx in range(len(self.parser.camtoworlds)):
                    stem = Path(self.parser.image_names[idx]).stem
                    ...
                    c2w = self.parser.camtoworlds[idx]  # (4, 4) normalized
                    cam_idx = self.parser.camera_ids[idx]
                    # Resize mask to the training resolution (K matrix is already scaled)
                    train_W, train_H = self.parser.imsize_dict[cam_idx]
                    if (H, W) != (train_H, train_W):
                        mask = np.array(
                            Image.fromarray(mask).resize(
                                (train_W, train_H), Image.NEAREST
                            )
                        )
                    mask_bin = mask > 0
                    H, W = mask_bin.shape

                    K_mat = self.parser.Ks_dict[cam_idx].copy()  # (3, 3)

                    # World-to-camera (in normalized space)
                    w2c = np.linalg.inv(c2w)
                    pts_cam = points @ w2c[:3, :3].T + w2c[:3, 3]  # (N, 3)
                    depths = pts_cam[:, 2]
                    pts_img = pts_cam @ K_mat.T
                    u = pts_img[:, 0] / np.maximum(pts_img[:, 2], 1e-10)
                    v = pts_img[:, 1] / np.maximum(pts_img[:, 2], 1e-10)

                    valid = (depths > 0) & (u >= 0) & (u < W) & (v >= 0) & (v < H)
                    vis_count[valid] += 1.0
                    hit_count[valid] += mask_bin[
                        v[valid].astype(int), u[valid].astype(int)
                    ].astype(np.float64)
                ...
                n_keep = keep.sum()
                print(
                    f"[MaskFilter] Kept {n_keep}/{N} points "
                    f"({100 * n_keep / N:.1f}%) with mask_ratio >= 0.3"
                )
                self.parser.points = points[keep]
                self.parser.points_rgb = self.parser.points_rgb[keep]
    ...
    def train(self):
        ...
        for step in pbar:
            ...
            if isinstance(self.cfg.strategy, DefaultStrategy):
                ...
                self._mask_contribution_audit(step)
            elif isinstance(self.cfg.strategy, MCMCStrategy):
                self.cfg.strategy.step_post_backward(
                    params=self.splats,
                    optimizers=self.optimizers,
    ...
    def eval(self, step: int, stage: str = "val"):
        ...
        for i, data in enumerate(valloader):
            ...
            if masks is not None:
                pixels_eval = pixels * masks[..., None]
                colors_eval = colors * masks[..., None]
            else:
                pixels_eval = pixels
                colors_eval = colors
            ...
            if world_rank == 0:
                ...
                if cfg.use_color_correction_metric:
                    if cfg.color_correct_method == "affine":
                        cc_colors = color_correct_affine(colors_eval, pixels_eval)
                    else:
                        cc_colors = color_correct_quadratic(colors_eval, pixels_eval)
    ...
    @torch.no_grad()
    def _mask_contribution_audit(self, step: int) -> None:
        """Suppress Gaussians whose projected centers rarely land inside the object mask.

        After split/clone, child Gaussians can be placed outside the mask region and
        receive zero gradient forever. This method projects each Gaussian's 3D center
        to K sampled training views and suppresses those that rarely fall inside the mask,
        letting the next _prune_gs call reclaim them.
        """
        cfg = self.cfg
        if cfg.mask_audit_every <= 0:
            return
        if not isinstance(cfg.strategy, DefaultStrategy):
            return
        strategy = cfg.strategy
        if not (strategy.refine_start_iter < step < strategy.refine_stop_iter):
            return
        if step % cfg.mask_audit_every != 0:
            return

        means = self.splats["means"]  # [N, 3]
        N = means.shape[0]
        hit_count = torch.zeros(N, device=self.device)
        vis_count = torch.zeros(N, device=self.device)

        n_views = min(cfg.mask_audit_views, len(self.trainset))
        view_indices = torch.randperm(len(self.trainset))[:n_views].tolist()

        for vi in view_indices:
            data = self.trainset[vi]
            c2w = data["camtoworld"].to(self.device)  # [4, 4]
            K_mat = data["K"].to(self.device)          # [3, 3]
            mask = data["mask"].to(self.device)        # [H, W] bool
            H, W = mask.shape

            w2c = torch.linalg.inv(c2w)
            pts_cam = means @ w2c[:3, :3].T + w2c[:3, 3]  # [N, 3]
            depths = pts_cam[:, 2]

            pts_img = pts_cam @ K_mat.T
            u = pts_img[:, 0] / pts_img[:, 2].clamp(min=1e-10)
            v = pts_img[:, 1] / pts_img[:, 2].clamp(min=1e-10)

            valid = (depths > 0) & (u >= 0) & (u < W) & (v >= 0) & (v < H)
            vis_count[valid] += 1.0

            u_valid = u[valid].long().clamp(0, W - 1)
            v_valid = v[valid].long().clamp(0, H - 1)
            hit_count[valid] += mask[v_valid, u_valid].float()

        # Gaussians never seen by any sampled view are kept (vis_count=0 → ratio=1.0)
        mask_ratio = torch.where(
            vis_count > 0,
            hit_count / vis_count.clamp(min=1.0),
            torch.ones(N, device=self.device),
        )
        suppress = (vis_count > 0) & (mask_ratio < cfg.mask_audit_ratio)
        n_suppress = suppress.sum().item()

        if n_suppress > 0:
            suppress_logit = torch.logit(
                torch.tensor(strategy.prune_opa * 0.5, device=self.device)
            )
            self.splats["opacities"].data[suppress] = suppress_logit

        if self.world_rank == 0:
            print(
                f"[MaskAudit] step={step}: suppressed {n_suppress}/{N} "
                f"({100 * n_suppress / max(N, 1):.1f}%) out-of-mask Gaussians"
            )
            self.writer.add_scalar("train/mask_audit_suppressed", n_suppress, step)
...
if __name__ == "__main__":
    ...
    CUDA_VISIBLE_DEVICES=0,1,2,3 python -m examples.customized_trainer default --steps_scaler 0.25
